...
class AddVC: UIViewController, UITextFieldDelegate, UITextViewDelegate, G8TesseractDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        //if photo library option was selected
    ...
    //output tesseract recognition progress
    ...
    func separate(strin: String) -> FoodItem {
        ...
        repeat
        {
            ...
            if(n < 13)
            {
                ...
                {
                    //indicate that it doesnt exist in input
                    found[n] = true
                    ...
                    if tokenArray.count > 0
                    {
                        for j in 0...tokenArray.count - 1
                        {
                            ...
                            if(n < 13)
                            {
                                ...
                                {
                                    // if current nutrient is a vitamin
                                    if((n == 8) || (n == 9)){
                                        //get next element
                                        if tokenArray.count > j+1
                                        {
                                            scannedString = tokenArray[j + 1]
                                            if( n == 8 && scannedString.uppercased() == "A"){
                                                //set nutrient name
                                                name = nutrientNames[n]
                                            }
                                            else if( n == 9 && scannedString.uppercased() == "C"){
                                                //set nutrient name
                                                name = nutrientNames[n]
                                            }
                                            else{
    ...
    func findNotFound(foundArray: [Bool]) -> Int {
        ...
        {
            //check if current nutrient is done being looked for
            if(foundArray[index] == false){
                return index
            }
        }
    ...
    func matchMajority(nutrient: String, scannedN: String) -> Bool {
        ...
        var count: Int //counter fot the number of letters to scan through
        ...
        //if there are characters to search through
        if(count > 0){  
